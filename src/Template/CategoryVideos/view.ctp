<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\CategoryVideo $categoryVideo
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Category Video'), ['action' => 'edit', $categoryVideo->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Category Video'), ['action' => 'delete', $categoryVideo->id], ['confirm' => __('Are you sure you want to delete # {0}?', $categoryVideo->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Category Videos'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Category Video'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="categoryVideos view large-9 medium-8 columns content">
    <h3><?= h($categoryVideo->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Category') ?></th>
            <td><?= h($categoryVideo->category) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Description') ?></th>
            <td><?= h($categoryVideo->description) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($categoryVideo->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($categoryVideo->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($categoryVideo->modified) ?></td>
        </tr>
    </table>
</div>
