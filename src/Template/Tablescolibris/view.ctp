<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Tablescolibri $tablescolibri
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Tablescolibri'), ['action' => 'edit', $tablescolibri->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Tablescolibri'), ['action' => 'delete', $tablescolibri->id], ['confirm' => __('Are you sure you want to delete # {0}?', $tablescolibri->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Tablescolibris'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Tablescolibri'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="tablescolibris view large-9 medium-8 columns content">
    <h3><?= h($tablescolibri->title) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Title') ?></th>
            <td><?= h($tablescolibri->title) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Text') ?></th>
            <td><?= h($tablescolibri->text) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Link') ?></th>
            <td><?= h($tablescolibri->link) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($tablescolibri->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($tablescolibri->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($tablescolibri->modified) ?></td>
        </tr>
    </table>
</div>
