<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Tablesadmin $tablesadmin
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Tablesadmin'), ['action' => 'edit', $tablesadmin->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Tablesadmin'), ['action' => 'delete', $tablesadmin->id], ['confirm' => __('Are you sure you want to delete # {0}?', $tablesadmin->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Tablesadmins'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Tablesadmin'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="tablesadmins view large-9 medium-8 columns content">
    <h3><?= h($tablesadmin->title) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Title') ?></th>
            <td><?= h($tablesadmin->title) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Text') ?></th>
            <td><?= h($tablesadmin->text) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Link') ?></th>
            <td><?= h($tablesadmin->link) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($tablesadmin->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($tablesadmin->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($tablesadmin->modified) ?></td>
        </tr>
    </table>
</div>
