...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $playlistReport->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $playlistReport->id)]
            )
        ?></li>
...
<div class="playlistReports form large-9 medium-8 columns content">
    <?= $this->Form->create($playlistReport) ?>
    <fieldset>
        <legend><?= __('Edit Playlist Report') ?></legend>
        <?php
