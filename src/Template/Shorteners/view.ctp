<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Shortener $shortener
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Shortener'), ['action' => 'edit', $shortener->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Shortener'), ['action' => 'delete', $shortener->id], ['confirm' => __('Are you sure you want to delete # {0}?', $shortener->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Shorteners'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Shortener'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="shorteners view large-9 medium-8 columns content">
    <h3><?= h($shortener->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Shortener') ?></th>
            <td><?= h($shortener->shortener) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Site') ?></th>
            <td><?= h($shortener->site) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($shortener->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($shortener->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($shortener->modified) ?></td>
        </tr>
    </table>
</div>
