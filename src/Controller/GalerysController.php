...
class GalerysController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) 
        {
            ...
            {
                $this->Flash->success2(__('The galery has been saved.'));
                ...
                {
                    $this->Flash->success2('A galeria foi criada com sucesso...');
                }
                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error2(__('The galery could not be saved. Please, try again.'));
        }
    ...
    public function delete($id = null)
    {
        ...
        {
            $this->Flash->success2(__('The galery has been deleted.'));
            ...
            if(is_dir($path))
            {
                ...
                if($dir->delete())
                {
                    $this->Flash->success2('Pasta' . $galery->title . 'excluída com sucesso!');
                }
                else
                {
                    $this->Flash->error2('Não foi possível excluir a pasta' . $galery->title . '!');
                }
            }
        } 
        else 
        {
            $this->Flash->error2(__('The galery could not be deleted. Please, try again.'));
        }
        return $this->redirect(['action' => 'index']);
    }
}
