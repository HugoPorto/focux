...
class VideosController extends AppController
{
    ...
    public function view($id = null)
    {
            ...
            $videosAdonis = $this->paginate($this->Videos->findByCategoryVideosId(37));
    ...
    public function edit($id = null, $editImage = null)
    {
        $this->maintenance();
        if($this->Roles->get($this->Auth->user()['roles_id'])->role == 'root' 
        || $this->Roles->get($this->Auth->user()['roles_id'])->role == 'admin')
        {
            $video = $this->Videos->get($id, [
                'contain' => []
            ]);
            ...
            else
            {
                if($editImage !== null)
                {
                    ...
                    $categoryVideos = $this->Videos->CategoryVideos->find('list', ['limit' => 200]);
                    $publicitys = $this->Videos->Publicitys->find('list', ['limit' => 200]);
                    $this->set(compact('video', 'categoryVideos', 'publicitys', 'editImage', 'courses'));
                }
                else
                {
                    $courses = $this->Videos->Courses->find('list', ['limit' => 200]);
    ...
    public function categorys()
    {
        ...
        $this->set(
            [
                'categoryVideos' => $this->paginate($categoryVideos),
                'footers' => $footers,
            ]);
    }
}
